#include"officially_assign2.hpp"
#include<iostream>
#include<cstring>
#include<iomanip>  
#include<vector>

extern int Child_Direction;
  ...
  // Part 4 - case 1
    {
        ...
        try {
                ...
                bst->comp = comp;
                tree_node *targetnode;
                std::cout<<"\033[41;11m wrong! \033[0m ";
                insert_into_BST(bst,55,&targetnode);
                insert_into_BST(bst,32,&targetnode);
                insert_into_BST(bst,90,&targetnode);
                insert_into_BST(bst,61,&targetnode);
            ...
            std::cout<<"\033[41;11m Error in part 4, case1!\033[0m";
            std::cout<<"\033[41;11m Happen in judgement step ";
            std::cout<<e<<". \033[0m\n";
        }
...
bool judge_bst(BST *bst, int &tree_c){
    if (bst->root != NULL){
        ...
        if (bst->root->l_child != NULL) {
            if (bst->comp(bst->root->data, 
            bst->root->l_child->data) < 0) return false;
            BST *new_bst = new BST;
            new_bst->comp = bst->comp;
            new_bst->root = bst->root->l_child;
        ...
        if (bst->root->r_child != NULL) {
            if (bst->comp(bst->root->data, 
            bst->root->r_child->data) > 0) return false;
            BST *new_bst = new BST;
            new_bst->comp = bst->comp;  
            new_bst->root = bst->root->r_child;
            judge_bst(new_bst, tree_c);
